...
export const localeSegmentRemoval = (segment: string = "") => {
  // Move to APP CONFIG
  const segmentLocale = segment?.substring(0, 5).replace(/eyc-/, "");
...
export const getCoreMediaUrl = (str: string) => {
  const coremedia ="coremedia://";
  const media ="media://";  
  const internalLink = str.includes(coremedia) || str.includes(media);
  
  let pathname = localeSegmentRemoval(
    str.includes(coremedia) ? str?.replace(coremedia, "") : str.includes(media) ? str?.replace(media, "") : str
  );
