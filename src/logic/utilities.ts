...
import { IAdapterPicture, IAdapterPictureRaw, ICrops } from "../models/ICrops";
...
export const getAdapterPicture = (obj: IAdapterPictureRaw[] = []): IAdapterPicture[] => {
  return obj.map((picture) => ({
    title: picture?.title,
    dataUri: picture?.data?.uri,
    uriTemplate: picture?.uriTemplate,
    crops: picture?.crops,
  }));
};
...
export const localeSegmentRemoval = (segment: string = "") => {
  // Move to APP CONFIG
  if (segment.includes(`${process.env.NEXT_PUBLIC_CM_SEGMENT}${defaultLocale}`))
...
export const removeCMTextHyphen = (str: string) => {
  return str === "-" || str === "<div>-</div>" || str === "<p>-</p>" || str === "<div><p>-</p></div>" ? "" : str;
};
