  ...
  try {
    ...
    const filteredLocalePaths: IFilteredLocalePaths[] | null = adaptedRes?.map(
      (path: IAdaptedRes) => {
        ...
        const localePath = locales.length > 1 ? `/${path.locale}` : "";
        const pagePath = page ? `/${page}/` : "/";
        const fullPath = isRoot
          ? `${basePath}${localePath}/`
          : `${basePath}${localePath}${pagePath}`;
    ...
    const isNotPROD = host?.includes("dev") || host?.includes("uat");

    if (isNotPROD) {
      // If in DEV or UAT, do not generate the sitemap, and return an empty response.
